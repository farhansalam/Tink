...
function thb_add_category_header_img() {
	...
	<div class="form-field">
		<label><?php _e( 'Header', 'bronx' ); ?></label>
		...
		<div style="line-height:60px;">
			...
			<button type="submit" class="thb_upload_header button"><?php _e( 'Upload/Add image', 'bronx' ); ?></button>
			<button type="submit" class="thb_remove_header button"><?php _e( 'Remove image', 'bronx' ); ?></button>
		</div>
		...
		<script type="text/javascript">
			...
			jQuery(document).on( 'click', '.thb_upload_header', function( event ){
				...
				header_file_frame = wp.media.frames.downloadable_file = wp.media({
					title: '<?php _e( 'Choose an image', 'bronx' ); ?>',
					button: {
						text: '<?php _e( 'Use image', 'bronx' ); ?>',
					},
					multiple: false
				});
...
function thb_edit_category_header_img( $term, $taxonomy ) {
	...
	<tr class="form-field">
		<th scope="row" valign="top"><label><?php _e( 'Header', 'bronx' ); ?></label></th>
		<td>
...
function woocommerce_product_cat_header_columns( $columns ) {
	...
	$new_columns['thumb'] = __( 'Image', 'bronx' );
	$new_columns['header'] = __( 'Header', 'bronx' );
	unset( $columns['cb'] );
	unset( $columns['thumb'] );
...
add_action('woocommerce_archive_description','thb_show_category_header');
function thb_show_category_header() {
